...
zip = 'special://home/addons/plugin.video.SemperFI-Wizard'
urlbase      =  'None'
mastercopy   =  ADDON.getSetting('mastercopy')
dialog = xbmcgui.Dialog()
urlupdate =  ""
updatename =  "semperFI_update"
backupdir    =  xbmc.translatePath(os.path.join('special://home/backupdir',''))
USB          =  xbmc.translatePath(os.path.join('special://home/addons/plugin.video.semperFI-Wizard',''))
...
def BACKUP():  
    ...
    if choice == 0:
	 return
      ...
      exclude_dirs_full =  ['backupdir','plugin.video.semperFI-Wizard','repository.semperFI-Builds','script.skinshortcuts','plugin.program.super.favourites']
 ...
 if os.path.exists(os.path.join(backupdir,'addon_data.zip')):   
	import time
        ...
        if choice == 0:
			...
			dp.create("[COLOR=blue][B]SemperFI[/B][/COLOR] Custom Builds Tool","Restoring",'', 'Please Wait')
...
def INDEX():
	...
	addDir('[COLOR lime][B]SemperFI Builds [/B][/COLOR]',BASEURL,20,ART+'schismwiz.png',FANART,'')
	addDir('[COLOR red][B]UPDATE[/B][/COLOR]','url',1,ART+'update.png',FANART,'')
...
def UPDATEMENU():
    linkupdate = OPEN_URL('https://archive.org/download/wizard_20160208_0400/wizard.txt').replace('\n','').replace('\r','')
    matchupdate = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(linkupdate)
    
    if not matchupdate:
	dialog = xbmcgui.Dialog()
        dialog.ok('[COLOR=lime][B]SemperFI[/B][/COLOR] [COLOR=white] Wizard[/COLOR]','NO Updates Currently Available','check back later.','')
	return
    ...
    link = OPEN_URL('https://archive.org/download/wizard_20160208_0400/wizard.txt').replace('\n','').replace('\r','')
...
def UPDATER():
	...
	return           
        ...
        if choice == 0:
            ...
            dp.create("[COLOR=lime][B]SemperFI[/B][/COLOR][COLOR=white] Wizard[/COLOR]","Updating",'', 'Please Wait')
...
def UPDATEUSERDATA():
	...
	INDEX()           
        ...
        REMOVE_EMPTY_FOLDERS()
	dialog = xbmcgui.Dialog()
	dialog.ok('[COLOR=lime][B]SemperFI[/B][/COLOR][COLOR=white] Wizard[/COLOR]','Addons Cleaned Now Install The Update from SchisM TV Builds','','')
	UPDATEMENU()
...
def WIZARDUPDATE():
	...
	# dialog = xbmcgui.Dialog()
        # dialog.ok('[COLOR=lime][B]SsmperFI[/B][/COLOR][COLOR=white] Updater[/COLOR] ','Please switch to the default Confluence skin','before proceeding.','')
    ...
    dp = xbmcgui.DialogProgress()
    dp.create("[COLOR=lime][B]SemperFI[/B][/COLOR][COLOR=white] Updater[/COLOR]","Downloading Update",'', 'Please Wait')
    ...
    extract.all(lib,addonfolder,dp)
    dialog = xbmcgui.Dialog()
    dialog.ok("[COLOR=lime][B]SemperFI[/B][/COLOR][COLOR=white] Updater[/COLOR]", "To save changes you now need to force close Kodi, DO not Forget to Clear Packages After Restart ... Press OK to force close Kodi")
    killxbmc()
def killxbmc():
...
def WipeXBMC():
    if skin!= "skin.confluence":
        dialog.ok('[COLOR=blue][B]SemperFI[/B][/COLOR][COLOR=green][/COLOR] Custom Builds Tool','Please switch to the default Confluence skin','before performing a wipe.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
        return
    else:
        ...
        if choice == 1:
            mybackuppath = xbmc.translatePath(os.path.join(backupdir,'SemperFI','My Builds'))
            ...
            exclude_dirs_full =  ['backupdir','plugin.video.semperFI','repository.semperFI']
            exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    ...
    if choice2 == 0:
        ...
        dp.create("[COLOR=blue][B]SemperFI[/B][/COLOR] Custom Builds Tool","Wiping Install",'', 'Please Wait')
        try:
    ...
    REMOVE_EMPTY_FOLDERS()
    REMOVE_EMPTY_FOLDERS()
    dialog.ok('[COLOR=blue][B]Semperfi [/B][/COLOR] Custom Builds Tool','Wipe Successful, please restart XBMC/Kodi for changes to take effect.','','')
...
def WipeInstall():
    if skin!= "skin.confluence":
        dialog.ok('[COLOR=blue][B]SemperFI[/B][/COLOR][COLOR=green]box[/COLOR] Tool','Please switch to the default Confluence skin','before performing a wipe.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")       
    else:
...
def WIZARD(name,url,description):
    if skin!= "skin.confluence":
	dialog = xbmcgui.Dialog()
        dialog.ok('[COLOR=lime][B]SemperFI[/B][/COLOR][COLOR=white]  Wizard[/COLOR] ','Please switch to the default Confluence skin','before proceeding.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
        return
    path = xbmc.translatePath(os.path.join('special://home/addons','packages'))
    dp = xbmcgui.DialogProgress()
    dp.create("[COLOR=lime][B]SemperFI[/B][/COLOR][COLOR=white] Wizard[/COLOR]","Downloading ",'', 'Please Wait')
    lib=os.path.join(path, name+'.zip')
    try:
       os.remove(lib)
    except:
    ...
    extract.all(lib,addonfolder,dp)
    dialog = xbmcgui.Dialog()
    dialog.ok("[COLOR=lime][B]SemperFI[/B][/COLOR][COLOR=white] Wizard[/COLOR]", "To save changes you now need to force close Kodi, Press OK to force close Kodi")
        ...
        # Count files and give option to delete
            if file_count > 0:
                ...
                if dialog.yesno("Delete Package Cache Files", str(file_count) + " files found", "Do you want to delete them?"):
                    ...
                    dialog = xbmcgui.Dialog()
                    dialog.ok("[COLOR lime][B]SemperFI[/B][/COLOR][COLOR white]Wizard[/COLOR]", "Packages Successfuly Removed", "")
    except: 
        dialog = xbmcgui.Dialog()
        dialog.ok("[COLOR lime][B]SemperFI[/B][/COLOR][COLOR white]Wizard[/COLOR]", "Sorry we were not able to remove Package Files", "")
...
def FRESHSTART(params):
    if skin!= "skin.confluence":
        dialog.ok('[COLOR lime][B]SemperFI[/B][/COLOR][COLOR white]Wizard[/COLOR] ','Please switch to the default Confluence skin','before performing a wipe.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
        return
    else:
    ...
    if choice2 == 0:
        ...
        dp.create("[COLOR lime][B]SemperFI[/B][/COLOR][COLOR white]Wizard[/COLOR]","Wiping Install",'', 'Please Wait')
        try:
            for root, dirs, files in os.walk(HOME,topdown=True):
                dirs[:] = [d for d in dirs if d not in EXCLUDES]
                for name in files:
    ...
    REMOVE_EMPTY_FOLDERS()
    REMOVE_EMPTY_FOLDERS()
    dialog.ok('[COLOR lime][B]SemperFI[/B][/COLOR][COLOR white]Wizard[/COLOR]','Wipe Successful, please restart XBMC/Kodi for changes to take effect.','','')
    killxbmc()
